...
export const Dashboard: React.FC<{ isDarkTheme: boolean; onThemeToggle: () => void }> = ({ isDarkTheme, onThemeToggle }) => {
  ...
  const deleteTrip = async (trip: TripRow) => {
    if (!window.confirm(`Delete "${trip.name}"? This will permanently remove its expenses, bookings, and participants.`)) return;
  ...
  return <div className={isDarkTheme ? 'theme-dark' : undefined}><main className="data-page">
    ...
    <div className="data-container">
      ...
      <div className="data-grid">
        ...
        <section className="data-panel glass-panel"><span className="marketing-eyebrow">YOUR TRIPS</span><h2>Open a ledger</h2><div className="trip-list">{trips.map((trip) => <div className="trip-list-row" key={trip.id}><button className="trip-list-open" onClick={() => navigate(`/dashboard?trip=${trip.id}`)}><strong>{trip.name}</strong><span>{trip.destination || 'No destination yet'} <ArrowRight className="inline w-3 h-3" /></span></button><button className="trip-list-delete" onClick={() => void deleteTrip(trip)} disabled={deletingTripId === trip.id} aria-label={`Delete ${trip.name}`} title={`Delete ${trip.name}`}><Trash2 className="w-4 h-4" /></button></div>)}{trips.length === 0 && <p className="empty-state">Create your first trip to get started.</p>}</div></section>
      </div>
    </div>
  </main></div>;
};
